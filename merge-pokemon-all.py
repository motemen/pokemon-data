...
"""
ポケモン統合データ作成スクリプト
data/merged/pokeapi_pkmn.tsv と data/merged/pokeapi_yakkuncom.tsv を
...
def main():
    ...
    df_pkmn = pd.read_csv("data/merged/pokeapi_pkmn.tsv", sep="\t")
    df_yakkuncom = pd.read_csv("data/merged/pokeapi_yakkuncom.tsv", sep="\t")

    print(f"Loaded {len(df_pkmn)} rows from pokeapi_pkmn.tsv", file=sys.stderr)
    print(
    ...
    pokeapi_columns = ["national_pokedex_number"] + [
        col for col in df_pkmn.columns if col.startswith("pokeapi_")
    ]
    ...
    # pkmnファイルからpokeapi_*カラムとpkmn_*カラムを取得
    pkmn_columns = pokeapi_columns + [
        col for col in df_pkmn.columns if col.startswith("pkmn_")
    ]
    df_pkmn_subset = df_pkmn[pkmn_columns]
    ...
    # pkmn_* カラム
    column_order.extend(
        [col for col in df_merged.columns if col.startswith("pkmn_")]
    )
    # yakkuncom_* カラム
    column_order.extend(
    ...
    # マッチしなかった行を報告
    no_pkmn_match = df_merged[df_merged["pkmn_name"].isnull()]
    no_yakkuncom_match = df_merged[df_merged["yakkuncom_name"].isnull()]

    if len(no_pkmn_match) > 0:
        print(
            f"WARNING: {len(no_pkmn_match)} rows have no PKMN match",
            file=sys.stderr,
        )
